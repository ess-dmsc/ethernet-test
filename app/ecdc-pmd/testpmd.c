...
struct fwd_engine *cur_fwd_eng = &rx_only_engine; /**< ECDC rxonly by default. */
				...
				*/
				...
				do {
					nb_rx = rte_eth_rx_burst(port_id, rxq,
						...
						printf("rx packet on port %u, queue %u\n", port_id, rxq);
						fflush(stdout);
					for (i = 0; i < nb_rx; i++)
