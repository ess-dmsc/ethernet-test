...
l2fwd_main_loop(void)
{
	...
	while (!force_quit) {
		...
		if (unlikely(diff_tsc > drain_tsc)) {
			...
			/* if timer is enabled */
			if (ecdc_options.timer_period > 0) {

				/* advance the timer */
				timer_tsc += diff_tsc;

				/* if timer has reached its timeout */
				if (unlikely(timer_tsc >= ecdc_options.timer_period)) {
...
l2fwd_parse_args(int argc, char **argv)
{
	...
	while ((opt = getopt_long(argc, argvopt, short_options,
		...
		case 'T':
			...
			ecdc_options.timer_period = timer_secs;
			break;
...
main(int argc, char **argv)
{
	...
	ecdc_options.timer_period *= rte_get_timer_hz();
